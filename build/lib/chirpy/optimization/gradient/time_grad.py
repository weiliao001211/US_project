...
class AdjointStateGrad(GradientEvaluator):
    ...
    def evaluate(
        ...
        time_end = time.time()
        print(
            f"[AdjointStateGrad] evaluate() done : |g|_max={g_max:.3e}, |g|_min={g_min:.3e} | done in {time_end - time_start:.2f}s"
        )
        return g_out
    ...
    def _single_grad(self, m: np.ndarray, res: np.ndarray, kind: str) -> np.ndarray:  # noqa: C901
        ...
        def _accum_a(u: np.ndarray, lam: np.ndarray) -> np.ndarray:
            """
            Gradient wrt attenuation a0, matching Perez-Liva et al. (JASA 2017, Eq. (18)).

            ∂e/∂a0(r) = ∫_0^T p†(r, T - t) [
                            2 c^{y-1} (-∇²)^{y/2} ∂_t p(r,t)
                          + 2 c^{y} tan(π y / 2) (-∇²)^{(y+1)/2} p(r,t)
                        ] dt
            """

            y = float(getattr(op.medium, "alpha_power", 1.5))  # 或 1.01，看你设定
            c0 = op.model_c.astype(np.float64, copy=False)

            # 系数部分
            K1 = 2.0 * np.power(c0, y - 1.0)  # 2 c^{y-1}
            K2 = 2.0 * np.power(c0, y) * np.tan(0.5 * np.pi * y)  # 2 c^{y} tan(π y / 2)

            # 1) (-∇²)^{y/2} p
            L1u = self._frac_laplacian_space(u, op, power=y / 2.0)
            dL1u_dt = self._d_dt(L1u, dt)  # ∂_t [(-∇²)^{y/2} p]

            # 2) (-∇²)^{(y+1)/2} p
            L2u = self._frac_laplacian_space(u, op, power=(y + 1.0) / 2.0)

            term1 = np.trapz(lam * dL1u_dt * K1[None], dx=dt, axis=0)
            term2 = np.trapz(lam * L2u * K2[None], dx=dt, axis=0)
    ...
    @staticmethod
    def _frac_laplacian_space(
        field: np.ndarray,
        op: WaveOperator,
        power: float,
    ) -> np.ndarray:
        """
        Approximate (-∇^2)^power acting on `field` in space via FFT.
        ...
        factor = k2_safe ** power
        factor[k2 < eps] = 0.0  # fractional Laplacian of constant = 0

        F = np.fft.fft2(field, axes=(1, 2))
        F *= factor[None, :, :]
        out = np.fft.ifft2(F, axes=(1, 2)).real
        return out