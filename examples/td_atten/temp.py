...
def main() -> None:
    ...
    c_raw = mat["C_true"]  # expected 2D array
    mat_atten = loadmat(DATA_DIR / "alpha_true.mat")
    alpha_raw = mat_atten["alpha_true"]  # expected 2D array

    img_grid = ImageGrid2D(nx=Nx, ny=Ny, dx=dx)

    c_img_true = ImageData(c_raw).downsample_to(new_grid=img_grid)
    c_true = c_img_true.array.astype(np.float32)

    alpha_img_true = ImageData(alpha_raw).downsample_to(new_grid=img_grid)
    # alpha_img_true.show(cmap='viridis')
    # exit()
    alpha_true = alpha_img_true.array.astype(np.float32)
    ...
    alpha0 = np.full((Ny, Nx), 0, dtype=np.float32)

    Fm1 = op.forward(model=alpha0, kind='alpha')
    print(op.medium)
    Fm2 = op.forward(model=alpha_true, kind='alpha')
    print(op.medium)
